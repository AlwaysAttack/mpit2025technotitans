"use strict";

Object.defineProperty(exports, "__esModule", {
  value: true
});
exports.useModuleListeners = void 0;
var _react = require("react");
var _reactNative = require("react-native");
/**
 * Copyright 2024 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *     https://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */

// A hook to manage event listeners for a specific Native Module,
// using the cross-platform NativeEventEmitter.
const useModuleListeners = (dispatcher, eventTypes, eventTransformer) => {
  const listenersRef = (0, _react.useRef)({});
  const eventEmitterRef = (0, _react.useRef)(null);
  const subsRef = (0, _react.useRef)({});
  const getEventEmitter = (0, _react.useCallback)(() => {
    if (!eventEmitterRef.current) {
      eventEmitterRef.current = new _reactNative.NativeEventEmitter(_reactNative.NativeModules[dispatcher]);
    }
    return eventEmitterRef.current;
  }, [dispatcher]);
  const removeAllNativeSubscriptions = (0, _react.useCallback)(() => {
    // Remove the specific subscriptions we created.
    Object.values(subsRef.current).forEach(sub => sub?.remove());
    subsRef.current = {};

    // As a safeguard, remove all listeners for the specified event types
    // from our emitter instance.
    eventTypes.forEach(eventType => {
      const name = String(eventType);
      getEventEmitter().removeAllListeners(name);
    });
  }, [eventTypes, getEventEmitter]);
  const updateListeners = (0, _react.useCallback)(() => {
    // Wrap listeners to multiplex events to all registered callbacks.
    const wrappedListeners = {};
    Object.keys(listenersRef.current).forEach(eventKey => {
      wrappedListeners[eventKey] = (...args) => {
        const transformedArgs = eventTransformer ? eventTransformer(eventKey, ...args) : args;
        listenersRef.current[eventKey]?.forEach(cb => cb(...transformedArgs));
      };
    });

    // Ensure all declared event types have a subscription, even if no JS listener is attached yet.
    eventTypes.forEach(eventType => {
      if (!wrappedListeners[eventType]) {
        wrappedListeners[eventType] = () => {}; // No-op handler
      }
    });
    removeAllNativeSubscriptions();

    // Subscribe to all events.
    eventTypes.forEach(eventType => {
      const name = String(eventType);
      const handler = wrappedListeners[eventType];
      const sub = getEventEmitter().addListener(name, handler);
      subsRef.current[name] = sub;
    });
  }, [eventTypes, eventTransformer, getEventEmitter, removeAllNativeSubscriptions]);
  const addListeners = listeners => {
    Object.keys(listeners).forEach(key => {
      const fn = listeners[key];
      if (!fn) return;
      listenersRef.current[key] = [...(listenersRef.current[key] || []), fn];
    });
    updateListeners();
  };
  const removeListeners = listeners => {
    Object.keys(listeners).forEach(key => {
      const fn = listeners[key];
      if (!fn) return;
      listenersRef.current[key] = listenersRef.current[key]?.filter(cb => cb !== fn);
      if (!listenersRef.current[key]?.length) {
        delete listenersRef.current[key];
      }
    });
    updateListeners();
  };
  const removeAllListeners = (0, _react.useCallback)(() => {
    listenersRef.current = {};
    updateListeners();
  }, [updateListeners]);
  (0, _react.useEffect)(() => {
    updateListeners();
    return () => {
      removeAllNativeSubscriptions();
      listenersRef.current = {};
    };
  }, [updateListeners, removeAllNativeSubscriptions]);
  return {
    addListeners,
    removeListeners,
    removeAllListeners
  };
};
exports.useModuleListeners = useModuleListeners;
//# sourceMappingURL=useModuleListeners.js.map